...
export default {
  async fetch(request) {
    ...
    const upstreamResponse = await fetch(upstreamUrl, {
      ...
      body:
        request.method !== "GET" && request.method !== "HEAD"
          ...
          : undefined,
      redirect: "follow",
      // Prevent edge caching of HTML/API (fixes "stuck old version" issues).
      cf: isStaticAsset
    ...
    // Debug headers: confirms which Worker served the request.
    responseHeaders.set("X-Cyber-Proxy", "cyber-oracle");
    responseHeaders.set("X-Cyber-Proxy-Static", isStaticAsset ? "1" : "0");
    responseHeaders.set("X-Cyber-Upstream", upstreamHost);
